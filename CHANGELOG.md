...
### Fixed
- Hassfest validation errors: manifest keys sorted (domain, name, then
  alphabetical) and removal of the unused top-level `selector` block in
  `strings.json` (labels are defined inline in the config flow; its `C`/`F`
  keys violated the lowercase translation-key rule).
