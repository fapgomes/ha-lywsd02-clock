...
- **Per-device entities**:
  ...
  - `sensor.<name>_battery` — battery percentage (diagnostic), updated whenever the battery read succeeds during a sync; otherwise the last known value is kept. Requires an active connection, which is why `xiaomi_ble` cannot provide it passively.
...
| `sensor.<name>_battery` | Battery percentage (diagnostic). Updated whenever the battery read succeeds during a sync; otherwise the last known value is kept. Requires a connection, which is why `xiaomi_ble` cannot provide it passively. |
